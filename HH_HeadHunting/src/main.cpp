...
BGSKeyword* kNPC = nullptr;
TESForm* marker = nullptr;
	...
	for (int i = 0; i <= 16; i += 16) {
		...
		if (form) {
			if (tempanims->object[i].skinned) {
				//logger::info("{}번 슬롯 발견", i);
				uint32_t tempId = form->formID;
				std::stringstream stream;
				stream << std::hex << tempId;
				std::string hexFormId = stream.str();
				//logger::info("{}번 슬롯 ID는 {}", i, hexFormId);
				...
				if (equipped->size() != 0 && (*equipped)[0].item.instanceData) {
					...
					TESObjectWEAP::InstanceData insData;
			
					float wSpeed = weap->weaponData.speed;
					logger::info("공속 {}", wSpeed);
				} else {
					logger::info("장착 무기를 못찾았음");
					return;
				}
				*/
				...
				if (form->GetPlayable(tempanims->object[i].parent.instanceData.get())) {
					...
					if (!ModelPath) {
						...
						double xDiff = ax - px;
						double yDiff = ay - py;
						double angleInRadians = -std::atan2(yDiff, xDiff) + (M_PI / 2.0) + GetRandomfloat(-0.25, 0.25);

						double heightDiff = pPoint.z - aPoint.z;
						double angleInZAxis = atan(heightDiff / pPoint.GetDistance(aPoint)) - GetRandomfloat(0.45, 0.75);
						if (angleInZAxis < -1.4) {
							angleInZAxis = -1.4;
						}
					...
					//logger::info("{}번 슬롯은 플레이어블이 아님", i);
					return;
				}
			} else {
				//logger::info("{}번 슬롯이 피부임", i);
				return;
			}
		} else {
			//logger::info("{}번 슬롯이 없음", i);
			return;
		}
	}
}
...
void OnF4SEMessage(F4SE::MessagingInterface::Message* msg)
{
	...
	case F4SE::MessagingInterface::kGameLoaded:
		...
		kNPC = (BGSKeyword*)dataHandler->LookupForm(0x00013794, "Fallout4.esm");
		marker = dataHandler->LookupForm(0x80B, "HH_HeadHunting.esp");
		break;
	}
}
